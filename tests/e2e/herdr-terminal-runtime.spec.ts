...
test('Herdr terminal detaches, survives Orca restart, reattaches, and closes explicitly', async ({
  ...
  const herdrBinary =
    process.env.ORCA_HERDR_TEST_BINARY ??
    path.resolve(
      process.cwd(),
      '..',
      'herdr',
  ...
  const herdrConfigRoot = os.tmpdir()
  const herdrConfigHome = mkdtempSync(path.join(herdrConfigRoot, 'orca-herdr-e2e-'))
  const herdrEnvironment = Object.fromEntries(
    Object.entries(process.env).filter(([key]) => !key.startsWith('HERDR_'))
  )
  if (process.platform !== 'win32') {
    herdrEnvironment.XDG_CONFIG_HOME = herdrConfigHome
  }
  let sessionName: string | null = null
  const marker = `HERDR_REATTACH_${Date.now()}`
  const stoppedMarker = path.join(herdrConfigHome, 'agent-stopped')
  const stoppedScript = path.join(herdrConfigHome, 'stopped-probe.js')
  writeFileSync(
    stoppedScript,
    `process.on('exit',()=>{const fs=require('fs');fs.writeFileSync(${JSON.stringify(stoppedMarker)},'stopped')});setInterval(()=>{},1000)`
  )
  const repoPath = testRepoPath
  const restartSessionEnv: Record<string, string> = {}
  if (process.platform !== 'win32') {
    restartSessionEnv.XDG_CONFIG_HOME = herdrConfigHome
  }
  const session = createRestartSession(testInfo, restartSessionEnv)
  ...
  try {
    ...
    const project = await first.page.evaluate(() => {
      const state = window.__store?.getState()
      return {
        project: state?.projects[0] ?? null,
        herdrSessionName: state?.settings.herdrSessionName
      }
    })
    sessionName =
      project.project && project.herdrSessionName
        ? herdrSessionNameForProject(project.project as Project, project.herdrSessionName)
        : null
      ...
      split = await client.call<{ split: RuntimeTerminalSplit }>('terminal.split', {
        ...
        command:
          process.platform === 'win32'
            ? `node ${stoppedScript}`
            : `sh -c 'trap "printf stopped > ${stoppedMarker}" EXIT HUP TERM; while :; do sleep 1; done'`
      })
    } catch (error) {
      const snapshot = execFileSync(herdrBinary, ['--session', sessionName!, 'api', 'snapshot'], {
        encoding: 'utf8',
