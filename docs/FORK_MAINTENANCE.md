...
A squash (2026-08-24) and a rebase (2026-08-25) each reset `main` to
`stablyai/orca:main` plus **one** Horca commit. Pre-rewrite tips are tagged
`horca/pre-squash-YYYYMMDD` / `horca/pre-rebase-YYYYMMDD`. After the rebase,
**do not force-push `main` again**. Restore the tag only if the rewrite must
be undone.

On 2026-08-20 a rebase of `main` deleted fork-maintenance work from PRs #7,
#8, and #9. The squash and this rebase are the documented exceptions to
"never rewrite main." Further rewrites are not allowed.
