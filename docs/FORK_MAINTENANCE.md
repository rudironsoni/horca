...
- Secret `FORK_SYNC_PAT` (or keep `UPSTREAM_SYNC_TOKEN`): Contents + Issues +
  PRs write; **Administration** write for rulesets and the auto-delete toggle;
  **workflows** write if syncing `.github/workflows`. Same token on checkout
  and the action. The default `GITHUB_TOKEN` cannot push workflow files and
  PRs it creates do not trigger PR CI.
