...
Pull requests into `main` run `.github/workflows/fork-overlay-guard.yml` (not
...
| `bump-horca-cask.yml` | Dispatch / `workflow_call` only. After a published Horca tag, rewrites `rudironsoni/homebrew-tap` `Casks/${cask_token}.rb` (version + sha256) and pushes. `cask_token` is `horca` or `horca@beta`. A missing tap file is copied from `config/horca-homebrew/Casks/`. Registers the tap checkout under Homebrew's `Library/Taps` before `brew style`/`brew audit` (Homebrew rejects bare cask paths). `FORK_SYNC_PAT` must be able to push to that tap. |
