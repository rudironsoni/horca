...
A one-time squash (2026-08-24) collapsed the previous merge-heavy fork
history. The pre-squash tip is tagged `horca/pre-squash-YYYYMMDD` on this
repository. After that rewrite, **do not force-push `main` again**. Restore
the tag only if the squash must be undone.

On 2026-08-20 a rebase of `main` deleted fork-maintenance work from PRs #7,
#8, and #9. The squash is the one documented exception to "never rewrite
main." Further rewrites are not allowed.
...
action: `secrets.FORK_SYNC_PAT` (falls back to `secrets.UPSTREAM_SYNC_TOKEN`).
...
Shepherd opens a `sync-conflict` issue and leaves `main` untouched. Resolve on
a throwaway branch; never merge `main` into upstream:
...
# Resolve conflicts. When both sides touched the same lines, take upstream
# unless the hunk is Horca identity (product name, protocol, state root).
...
These need repository admin on GitHub. Automation tokens on this fork do not
have that access.

- Secret `FORK_SYNC_PAT` (or keep `UPSTREAM_SYNC_TOKEN`): Contents + Issues +
  PRs write; **workflows** write if syncing `.github/workflows`. Same token on
  checkout and the action. The default `GITHUB_TOKEN` cannot push workflow
  files and PRs it creates do not trigger PR CI.
- Labels: `sync-conflict` (cleanup), `sync-bot` (optional because
  `monitor_all_prs` is true), `backport-to-*` as needed.
- Rulesets on `main`: **block force-push** and **restrict deletions**. Do
  **not** require a pull request (Shepherd must push merge commits). Do not
  require the branch to be up to date. No bypass after the squash. Stop
  auto-deleting long-lived head branches.
- Delete `upstream-main` after the first green Fork Shepherd run.
