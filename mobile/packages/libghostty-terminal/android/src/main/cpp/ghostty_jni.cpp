...
JNIEXPORT jbyteArray JNICALL
Java_expo_modules_libghostty_GhosttyVt_nativeEncodeMouseClick(
    JNIEnv* env, jobject, jlong handle, jfloat x, jfloat y, jint screenWidth,
    jint screenHeight, jint cellWidth, jint cellHeight) {
  auto* session = fromHandle(handle);
  if (session == nullptr || screenWidth <= 0 || screenHeight <= 0 ||
  ...
  size.screen_width = static_cast<uint32_t>(screenWidth);
  size.screen_height = static_cast<uint32_t>(screenHeight);
  size.cell_width = static_cast<uint32_t>(cellWidth);
  size.cell_height = static_cast<uint32_t>(cellHeight);
  ...
  ghostty_mouse_event_set_mods(session->mouseEvent, 0);
  ghostty_mouse_event_set_position(session->mouseEvent, GhosttyMousePosition{x, y});

  std::vector<char> output;
  output.reserve(64);
  for (const GhosttyMouseAction action :
       {GHOSTTY_MOUSE_ACTION_PRESS, GHOSTTY_MOUSE_ACTION_RELEASE}) {
    ghostty_mouse_event_set_action(session->mouseEvent, action);
    const bool pressed = action == GHOSTTY_MOUSE_ACTION_PRESS;
    ...
    const GhosttyResult result = ghostty_mouse_encoder_encode(
        session->mouseEncoder, session->mouseEvent, buffer, sizeof(buffer), &written);
    if (result != GHOSTTY_SUCCESS) return nullptr;
