# Orca libghostty terminal

Ghostty terminal view for Expo / React Native, powered by
[libghostty](https://ghostty.org) — on iOS via
[Lakr233/libghostty-spm](https://github.com/Lakr233/libghostty-spm) 1.4.11, on
...
allow the workspace package in `pnpm-workspace.yaml`:

```yaml
allowBuilds:
  '@orca/libghostty-terminal@file:packages/libghostty-terminal': true
```
...
```tsx
import { TerminalView, TerminalViewRef } from '@orca/libghostty-terminal'
...
| `onTap`               | Clean terminal tap with the visible row text and tapped column when available.                                                                                                                                                                                                                                  |
| `keyboardEnabled`     | Allows the native surface to open its own keyboard. Orca disables this because its live and buffered input controls own keyboard focus.                                                                                                                                                                         |
...
| `resetWithText(text)` | Replace terminal state with a serialized ANSI snapshot.             |
| `pasteText(text)`     | Paste through Ghostty, including bracketed-paste and safety checks. |
...
and the XCFramework checksum; `pnpm sync-vendor` re-syncs them.
