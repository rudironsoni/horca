    ...
    (handle: string) => {
      ...
      if (!covered) {
        if (!getTerminalRef(handle)) {
          logSkippedGate('no-native-ref')
          return
        }
        if (!nativeReadyHandlesRef.current.has(handle)) {
          logSkippedGate('native-not-ready')
          return
        }
      }
      ...
      const unsub = subscribeMobileTerminalSafely(
        ...
        (result) => {
          ...
          // Keep the subscription as the input-floor lease without mutating the covered surface.
          if (
          ...
          if (data.type === 'scrollback') {
            ...
            void ref.resetWithText(initialData)
            ...
            void dataRef.writeText(data.chunk as string)
            ...
            if (serialized != null) {
              void getTerminalRef(handle)?.resetWithText(serialized)
            }
            if (data.displayMode) {
              const displayMode = data.displayMode as MobileDisplayMode
              setTerminalModes((prev) =>
