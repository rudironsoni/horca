...
// (format ansi) diffed by revision, pumped on pane_output_changed plus a slow
// poll as a backstop. write -> pane.send_input, resize -> absolute via the
// pane.layout rect + relative pane.resize, closed -> pane_exited.
export function createHerdrSocketTerminalController(
  ...
  const readFrame = async (): Promise<void> => {
    ...
    try {
      ...
      if (released || result.revision === revision) {
        return
      }
