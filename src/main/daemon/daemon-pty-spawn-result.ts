...
export abstract class DaemonPtySpawnResult extends DaemonPtySpawnRequest {
  protected async finishSpawn(
    ...
    // Why kitty flags ride beside the payload, not inside it: the snapshot reaches renderer terminals where POST_REPLAY_REATTACH_RESET's kitty reset must win (terminal-query-authority.md §kitty).
