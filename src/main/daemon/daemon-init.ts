...
export async function initDaemonPtyProvider(
  ...
  store?.onSettingsChanged((updates) => {
    if (!('terminalBackendDefault' in updates)) {
      return
    }
    if (updates.terminalBackendDefault === 'herdr') {
      if (!herdrProvider && herdrStore) {
        herdrProvider = createLocalHerdrPtyProvider(adapter ?? undefined, herdrStore)
  ...
  const herdrBackendActive = store?.getSettings().terminalBackendDefault === 'herdr'
  console.error(
