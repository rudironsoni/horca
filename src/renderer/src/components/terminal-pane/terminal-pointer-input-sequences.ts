// Why: terminal flags pointer-derived bytes as user input alongside keystrokes; callers
// that must treat pointer input differently need to recognise it by shape.

/** True for an terminal mouse report (X10 `CSI M` or SGR `CSI <`): pointer input, never a keystroke. */
...
/** True for the bare cursor up/down terminal synthesises per wheel notch when the active buffer has no scrollback. */
