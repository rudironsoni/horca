import { getAcceptedDeepLinkProtocols } from '../../../shared/distribution-identity'
...
export function parseWebPairingInput(input: string): WebPairingOffer | null {
  ...
  try {
    const pairingPrefixes = [...getAcceptedDeepLinkProtocols()].map(
      (protocol) => `${protocol}//`
    )
...
export function readPairingInputFromLocation(location: Location): string | null {
  ...
  const pairingPrefixes = [...getAcceptedDeepLinkProtocols()].map(
    (protocol) => `${protocol}//pair`
  )
  if (pairingPrefixes.some((prefix) => hash.startsWith(prefix))) {
    return hash
  }
