...
describe('Horca sync workflow', () => {
  ...
  it('stops the sync on rebase conflicts instead of auto-continuing', () => {
    const rebaseStep = workflow.jobs.candidate.steps.find(
      (step) => step.name === 'Rebase patch stack'
    )
    const workflowSource = readFileSync('.github/workflows/horca_sync.yml', 'utf8')

    expect(workflowSource).not.toContain('continue-sync-rebase')
    expect(rebaseStep.run).toContain('git rebase --abort')
...
describe('Horca release workflow', () => {
  ...
  it('publishes with the Horca Maintenance app instead of a personal PAT', () => {
    ...
    expect(publishStep.env.GITHUB_TOKEN).toBe('${{ steps.app.outputs.token }}')
    expect(publishStep.uses).toContain('softprops/action-gh-release@')
    expect(publishStep.with.tag_name).toBe('${{ needs.metadata.outputs.tag }}')
    expect(publishStep.with.files).toBe('artifacts/*')
