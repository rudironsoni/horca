...
function withEnv(env, assert) {
  ...
  const downstreamPath = require.resolve('../electron-builder-downstream.cjs')
  ...
  try {
    ...
    delete require.cache[downstreamPath]
...
describe('electron-builder downstream distribution config', () => {
  ...
  it('applies the downstream identity to macOS and Windows packaging', () => {
    withDownstreamEnv((config) => {
      ...
      expect(config.linux.executableName).toBe('horca-ide')
      expect(config.linux.desktop.entry.StartupWMClass).toBe('horca')
      expect(JSON.stringify(config.linux.extraResources)).toContain('"to":"bin/horca"')
      expect(config.deb.packageName).toBe('horca-ide')
      expect(config.rpm.packageName).toBe('horca-ide')
      expect(config.deb.afterInstall.endsWith('after-install-horca.sh')).toBe(true)
      expect(config.appImage.artifactName).toBe('horca-linux-${arch}.${ext}')
    })
  })
