...
export function isPaneWebglContextLost(pane: ManagedPaneInternal): boolean {
  try {
    const renderer = (requireXtermPaneState(pane.terminal).webglAddon as unknown as XtermWebglAddonInternals | null)?._renderer
...
export function disposeWebgl(
  ...
  if (!requireXtermPaneState(pane.terminal).webglAddon) {
    return
  }
  releaseXtermWebglContext(requireXtermPaneState(pane.terminal).webglAddon)
  try {
    requireXtermPaneState(pane.terminal).webglAddon?.dispose()
  } catch {
    /* ignore */
  }
...
function releaseXtermWebglContext(webglAddon: ReturnType<typeof requireXtermPaneState>["webglAddon"]): void {
  try {
...
export function clearWebglTextureAtlas(pane: ManagedPaneInternal): void {
  ...
  try {
    ...
    requireXtermPaneState(pane.terminal).webglAddon?.clearTextureAtlas()
  } catch {
    /* ignore — pane may have been disposed in the meantime */
  }
}
...
export function attachWebglAfterFitIfMissing(pane: ManagedPaneInternal): void {
  ...
  if (
    !requireXtermPaneState(pane.terminal).webglAddon &&
...
export function attachWebgl(pane: ManagedPaneInternal): void {
  ...
  try {
    ...
    requireXtermPaneState(pane.terminal).term.loadAddon(addon)
    requireXtermPaneState(pane.terminal).webglAddon = addon
