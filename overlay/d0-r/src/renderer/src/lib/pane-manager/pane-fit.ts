...
function performSafeFit(pane: ManagedPane): boolean {
  ...
  const captureScrollForFit = (): void => {
    ...
    pinnedScrollState =
      scrollIntent?.kind === 'pinnedViewport' ? captureScrollState(requireXtermPaneState(pane.terminal).term) : null
  ...
  try {
    ...
    if (override) {
      if (pane.terminal.cols !== override.cols || pane.terminal.rows !== override.rows) {
        ...
        resumePendingFitScrollRestoreAfterFit(requireXtermPaneState(pane.terminal).term)
      }
      return true
    }
    ...
    requireXtermPaneState(pane.terminal).fitAddon.fit()
    return true
  } catch {
    ...
    if (shouldRestoreScroll) {
      try {
        if (resumePendingFitScrollRestoreAfterFit(requireXtermPaneState(pane.terminal).term)) {
          ...
          restoreScrollStateAfterFit(requireXtermPaneState(pane.terminal).term, state, {
            onRestored: () => {
