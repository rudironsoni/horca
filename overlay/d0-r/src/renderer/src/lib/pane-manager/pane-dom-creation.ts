...
import { XtermPaneTerminal } from './xterm-renderer/xterm-pane-terminal'
...
export function createPaneDOM(
  ...
  const terminal = new XtermPaneTerminal({
    terminalOptions: terminalOpts,
    onLinkClick: options.onLinkClick
      ? (event, uri) => options.onLinkClick!(id, event, uri)
      : undefined,
    linkHover: (uri) => {
