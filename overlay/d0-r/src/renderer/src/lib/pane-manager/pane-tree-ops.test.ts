...
describe('safeFit', () => {
  ...
  it('coalesces fits until replay parsing and scroll restoration complete', async () => {
    ...
    vi.mocked(requireXtermPaneState(pane.terminal).fitAddon.fit).mockImplementation(() => {
  ...
  it('retains an authoritative continuation until a later measurable fit succeeds', async () => {
    ...
    vi.mocked(requireXtermPaneState(pane.terminal).fitAddon.proposeDimensions).mockReturnValue(undefined)
    const continuation = vi.fn()

    const pending = safeFitAndThen(pane, 'pty-resize', continuation)

    expect(continuation).not.toHaveBeenCalled()
    vi.mocked(requireXtermPaneState(pane.terminal).fitAddon.proposeDimensions).mockReturnValue({ cols: 100, rows: 32 })
    safeFit(pane)
