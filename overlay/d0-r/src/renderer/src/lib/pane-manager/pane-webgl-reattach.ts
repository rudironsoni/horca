...
export function reattachWebglIfNeeded(pane: ManagedPaneInternal): void {
  if (pane.gpuRenderingEnabled && !requireXtermPaneState(pane.terminal).webglAddon && clearPaneWebglContextLossForRetry(pane)) {
    attachWebgl(pane)
  }
}

export function rebuildAttachedWebgl(pane: ManagedPaneInternal): void {
  if (!requireXtermPaneState(pane.terminal).webglAddon || pane.webglDisabledAfterContextLoss) {
    return
  }
