...
function runAfterNormalBuffer(
  ...
  disposable = requireXtermPaneState(pane.terminal).term.buffer.onBufferChange((buffer: IBuffer) => {
...
function restoreCapturedScrollState(
  ...
  restoreScrollState(requireXtermPaneState(pane.terminal).term, scrollState)
...
export function scheduleSplitScrollRestore(
  ...
  const firstRafId = requestAnimationFrame(() => {
    ...
    const secondRafId = requestAnimationFrame(() => {
      ...
      restoreScrollState(requireXtermPaneState(live.terminal).term, scrollState)
