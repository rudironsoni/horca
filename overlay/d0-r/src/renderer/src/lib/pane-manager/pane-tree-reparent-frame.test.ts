...
function createPane(id: number, container = createMockElement('pane')): ManagedPaneInternal {
  const leafId = `${id}1111111-1111-4111-8111-111111111111` as ManagedPaneInternal['leafId']
  const { pane } = createXtermPaneTestHarness({
    terminal: {} as PaneTerminal,
    state: {
      webglAddon: { dispose: vi.fn() } as unknown as XtermPaneState['webglAddon']
    },
    pane: {
      id,
      leafId,
