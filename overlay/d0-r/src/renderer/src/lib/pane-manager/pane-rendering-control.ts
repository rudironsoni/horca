...
export function suspendPaneRendering(
  ...
  for (const pane of suspended) {
    ...
    suspendTerminalCursorBlink(requireXtermPaneState(pane.terminal).term)
  }
...
export function resumePaneRendering(
  ...
  for (const pane of panes) {
    ...
    resumeTerminalCursorBlink(requireXtermPaneState(pane.terminal).term)
    ...
    if (requireXtermPaneState(pane.terminal).webglAddon && isPaneWebglContextLost(pane)) {
      disposeWebgl(pane)
    }
    if (rebuildDeferred && requireXtermPaneState(pane.terminal).webglAddon) {
