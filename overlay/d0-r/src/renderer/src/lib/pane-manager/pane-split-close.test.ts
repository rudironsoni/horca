...
import { requireXtermPaneState, type XtermPaneState } from './xterm-renderer/xterm-pane-state'
...
function createPane(id: number, webglAddon: unknown): ManagedPaneInternal {
  ...
  const { pane } = createXtermPaneTestHarness({
    ...
    state: {
      webglAddon: (webglAddon ?? null) as XtermPaneState['webglAddon']
    },
    pane: {
      id,
      leafId: TEST_LEAF_ID,
...
describe('splitManagedPane', () => {
  ...
  it('prepares every pane under a moved mounted subtree for split reparenting', () => {
    ...
    expect(result?.id).toBe(newPane.id)
    expect(captureScrollState).toHaveBeenCalledWith(
      requireXtermPaneState(fallbackPane.terminal).term
    )
    expect(captureScrollState).toHaveBeenCalledWith(
      requireXtermPaneState(siblingPane.terminal).term
    )
