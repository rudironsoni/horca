...
function createPane(): ManagedPaneInternal {
  ...
  return registerMockXtermPaneState(pane)
}
...
describe('attachWebgl', () => {
  ...
  it('keeps a pane on the DOM renderer after WebGL context loss', () => {
    ...
    attachWebgl(pane)
    expect(requireXtermPaneState(pane.terminal).term.loadAddon).toHaveBeenCalledTimes(1)
    expect(webglMock.contextLossHandler).not.toBeNull()
    vi.mocked(pane.terminal.refresh).mockClear()

    webglMock.contextLossHandler?.()

    expect(requireXtermPaneState(pane.terminal).webglAddon).toBeNull()
    expect(pane.webglDisabledAfterContextLoss).toBe(true)
    expect(requireXtermPaneState(pane.terminal).fitAddon.fit).toHaveBeenCalledTimes(1)
  ...
  it('does not attach WebGL while initial rendering is deferred', () => {
    ...
    expect(requireXtermPaneState(pane.terminal).webglAddon).toBeNull()
    expect(requireXtermPaneState(pane.terminal).term.loadAddon).not.toHaveBeenCalled()
  })
  ...
  it('uses WebGL rendering for auto GPU acceleration on non-Linux platforms', () => {
    ...
    expect(requireXtermPaneState(pane.terminal).webglAddon).not.toBeNull()
    expect(requireXtermPaneState(pane.terminal).term.loadAddon).toHaveBeenCalledTimes(1)
  })
  ...
  it('keeps auto-mode panes on WebGL after complex-script output', () => {
    ...
    attachWebgl(pane)
    expect(requireXtermPaneState(pane.terminal).term.loadAddon).toHaveBeenCalledTimes(1)
    vi.mocked(requireXtermPaneState(pane.terminal).term.loadAddon).mockClear()
    ...
    expect(requireXtermPaneState(pane.terminal).fitAddon.fit).not.toHaveBeenCalled()
...
describe('attachLigatures', () => {
  it('refreshes existing rows after loading the ligatures addon', () => {
    ...
    expect(requireXtermPaneState(pane.terminal).term.loadAddon).toHaveBeenCalledTimes(1)
    expect(pane.terminal.refresh).toHaveBeenCalledWith(0, 23)
    expect(requireXtermPaneState(pane.terminal).ligaturesAddon).not.toBeNull()
  })

  it('defers a retained WebGL rebuild while hidden', () => {
    const pane = createPane()
    const retainedAddon = { dispose: vi.fn() } as never
    requireXtermPaneState(pane.terminal).webglAddon = retainedAddon
    pane.webglAttachmentDeferred = true

    attachLigatures(pane)

    expect(requireXtermPaneState(pane.terminal).webglAddon).toBe(retainedAddon)
...
describe('openTerminal — addon and provider wiring', () => {
  ...
  function createOpenTerminalHarness(): {
    ...
    registerMockXtermPaneState(pane)
  ...
  it('builds one initial WebGL atlas with ligatures and still rebuilds on a live toggle', async () => {
    ...
    expect(requireXtermPaneState(pane.terminal).ligaturesAddon).not.toBeNull()
    expect(requireXtermPaneState(pane.terminal).webglAddon).not.toBeNull()
    const addons = vi.mocked(requireXtermPaneState(pane.terminal).term.loadAddon).mock.calls.map(([addon]) => addon)
    expect(addons.indexOf(requireXtermPaneState(pane.terminal).ligaturesAddon!)).toBeLessThan(addons.indexOf(requireXtermPaneState(pane.terminal).webglAddon!))
    ...
    expect(requireXtermPaneState(pane.terminal).ligaturesAddon).toBeNull()
  })
  ...
  it('joins RTL runs only while the pane has a live WebGL addon', () => {
    ...
    requireXtermPaneState(pane.terminal).webglAddon = {} as never
