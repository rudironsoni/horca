...
function clearTerminalSearch(terminal: PaneTerminal | null): void {
  if (!terminal) {
    return
  }
...
export default function TerminalSearch({
  ...
  const findNext = useCallback(() => {
    if (terminal && requestQuery) {
      safeFind(
        (term, options) => terminal.findNext(term, options),
        requestQuery,
        searchOptions()
      )
    }
  }, [terminal, requestQuery, searchOptions])

  const findPrevious = useCallback(() => {
    if (terminal && requestQuery) {
      safeFind(
        (term, options) => terminal.findPrevious(term, options),
        requestQuery,
        searchOptions()
      )
    }
  ...
  }, [requestQuery, terminal, isOpen, caseSensitive, regex, searchStateRef, searchOptions])
