...
export function bindForegroundOutputRefresh(session: ConnectPanePtySession): void {
  ...
  session.beforeTerminalOutputWrite = function (data: string): void {
    ...
    ensureArabicShapingJoinerForText(requireXtermPaneState(session.pane.terminal).term, data)
    recordTerminalOutput(requireXtermPaneState(session.pane.terminal).term)
  }
