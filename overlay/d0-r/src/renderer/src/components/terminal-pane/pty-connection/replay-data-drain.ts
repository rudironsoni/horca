  ...
  session.sendFocusedReattachFocusInAfterReplay = (
    ...
    void waitForTerminalOutputParsed(session.pane.terminal).then(() => {
      ...
      if (
        ...
        if (parsedViewportShowsParkedCursorAgentScreen(requireXtermPaneState(session.pane.terminal).term) === false) {
      ...
      const sendFocusMode = terminalHasFocusReportingEnabled(requireXtermPaneState(session.pane.terminal).term)
