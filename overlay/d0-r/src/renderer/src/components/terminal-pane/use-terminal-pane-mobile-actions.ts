  ...
  const getPrimarySelectionMiddleClickPane = useCallback(
    (target: EventTarget | null) => {
      ...
      const clickedPane =
        ...
        manager.getPanes()[0]
      if (!clickedPane || requireXtermPaneState(clickedPane.terminal).term.modes.mouseTrackingMode !== 'none') {
        return null
      }
      return clickedPane
    },
      ...
      void readPrimarySelectionText().then(async (text) => {
        ...
        const execution = await executeTerminalPastePlan(plan, {
          pasteText: (pasteText, pasteOptions) =>
            pasteTerminalText(requireXtermPaneState(clickedPane.terminal).term, pasteText, pasteOptions),
