...
describe('resumeTerminalVisibility reveal repaint', () => {
  ...
  it('captures native trim movement before enforcing viewport intent', async () => {
    ...
    const manager = createManager()
    manager.getPanes.mockReturnValue([attachXtermPaneTestState({ terminal } as never)])
  ...
  it('resets each pane linkifier hover cache on reveal so links recover without a scroll', () => {
    ...
    const manager = createManager()
    manager.getPanes.mockReturnValue([attachXtermPaneTestState({ terminal: first } as never), attachXtermPaneTestState({ terminal: second } as never)])
  ...
  it('rebuilds the atlas synchronously when a heavy reveal repaired a dpr mismatch', async () => {
    ...
    const pane = attachXtermPaneTestState({ terminal: {} } as never)
    const manager = createManager()
  ...
  it('checks each pane for a stale WebGL backing on a light tab reveal', () => {
    const first = attachXtermPaneTestState({ terminal: { name: 'pane-a' } } as never)
    const second = attachXtermPaneTestState({ terminal: { name: 'pane-b' } } as never)
    const manager = createManager()
  ...
  it('keeps a genuinely-hovered link intact on window wake recovery', () => {
    ...
    const manager = createManager()
    manager.getPanes.mockReturnValue([attachXtermPaneTestState({ terminal: hovered } as never)])
