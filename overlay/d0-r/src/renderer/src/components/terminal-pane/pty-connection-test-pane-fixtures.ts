import { registerXtermPaneState } from '../../lib/pane-manager/xterm-renderer/xterm-pane-state'
import type { PaneTerminal } from '../../lib/pane-manager/pane-terminal'
...
export function createPane(paneId: number): MockPane {
  ...
  registerXtermPaneState(terminal as unknown as PaneTerminal, {
    term: terminal as never,
    fitAddon: fitAddon as never,
    searchAddon: { findNext: vi.fn(), findPrevious: vi.fn(), clearDecorations: vi.fn(), dispose: vi.fn() } as never,
    serializeAddon: { serialize: vi.fn(() => ''), dispose: vi.fn() } as never,
