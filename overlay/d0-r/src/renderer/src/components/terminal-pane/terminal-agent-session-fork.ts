...
export function prepareAgentSessionForkFromPane({
  ...
  const prompt = buildAgentSessionForkPrompt({
    capturedText: requireXtermPaneState(pane.terminal).serializeAddon.serialize({ scrollback: 800 }),
    sourceLabel: paneKey,
    agentLabel: agent
  })
...
export async function copyAgentSessionContextFromPane(pane: ManagedPane): Promise<boolean> {
  const transcript = buildBoundedSessionTranscript(
    requireXtermPaneState(pane.terminal).serializeAddon.serialize({ scrollback: 800 })
  )
  if (!transcript) {
    toast.error(
      translate(
