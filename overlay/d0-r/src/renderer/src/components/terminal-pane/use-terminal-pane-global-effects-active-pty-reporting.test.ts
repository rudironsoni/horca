...
describe('useTerminalPaneGlobalEffects', () => {
  ...
  function makeActivePtyManager(): {
    ...
    return {
      getPanes: vi.fn(() => [attachXtermPaneTestState({ id: 1, terminal: { name: 'terminal-a' } } as never)]),
      ...
      getActivePane: vi.fn(() => (attachXtermPaneTestState({ id: 1, terminal: { name: 'terminal-a' } } as never)))
    }
  }
