...
type ShutdownPane = Pick<ManagedPane, 'id' | 'leafId' | 'terminal'>
...
function serializeWithinSessionScrollbackByteLimit(
  ...
  for (let probe = 0; probe < MAX_SCROLLBACK_FIT_PROBES || best === null; probe += 1) {
    ...
    const attempt = serializeWithAbsoluteCursor(requireXtermPaneState(pane.terminal).serializeAddon, pane.terminal, {
      scrollback: rows
    })
  ...
  if (captureBuffers) {
    for (const pane of panes) {
      try {
        ...
        let serialized = serializeWithAbsoluteCursor(requireXtermPaneState(pane.terminal).serializeAddon, pane.terminal, {
          scrollback
        })
