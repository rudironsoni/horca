...
export function useTerminalScrollVisibilityMemory({
    ...
    (useRememberedSnapshots: boolean): Map<number, ScrollState> => {
      ...
      return new Map(
        manager.getPanes().map((pane) => {
          ...
          const state = captureScrollState(requireXtermPaneState(pane.terminal).term)
          if (!useRememberedSnapshots || !remembered) {
            visibleScrollSnapshotsRef.current.set(pane.id, {
              scrollState: state,
              outputEpoch: getTerminalOutputEpoch(requireXtermPaneState(pane.terminal).term)
            })
          }
  ...
  const applyPendingFollowOutputRequests = useCallback((): boolean => {
    ...
    for (const pane of manager.getPanes()) {
      ...
      flushTerminalOutput(requireXtermPaneState(pane.terminal).term, { maxChars: FOLLOW_OUTPUT_FLUSH_CHARS })
      const currentEpoch = getTerminalOutputEpoch(requireXtermPaneState(pane.terminal).term)
      const hasNewOutput = previous ? currentEpoch > previous.outputEpoch : currentEpoch > 0
      if (hasNewOutput) {
        if (getTerminalScrollIntentKind(requireXtermPaneState(pane.terminal).term) === 'followOutput') {
          cancelDeferredScrollRestore(requireXtermPaneState(pane.terminal).term)
          markTerminalFollowOutput(requireXtermPaneState(pane.terminal).term)
          pane.terminal.scrollToBottom()
          didScroll = true
        }
        rememberVisibleScrollSnapshot(pane.id, requireXtermPaneState(pane.terminal).term)
      }
