...
export function installTerminalPaneLinkHandling(context: PaneLinkContext): void {
  ...
  const linkPointerGesture = installTerminalLinkPointerGesture(requireXtermPaneState(pane.terminal).term)
  ...
  refs.terminalHandleLinkDisposablesRef.current.set(
    ...
    pane.terminal.registerLinkProvider(
      createTerminalHandleLinkProvider({
        getTerminal: () => {
          const found = managerRef.current?.getPanes().find((candidate) => candidate.id === pane.id)?.terminal ?? null
          return found ? requireXtermPaneState(found).term : null
        },
  ...
  refs.linkifierClickPrimingDisposablesRef.current.set(
    pane.id,
    installTerminalLinkifierClickPriming(requireXtermPaneState(pane.terminal).term)
  )
  refs.fileLinkClickFallbackDisposablesRef.current.set(
    pane.id,
    installFilePathLinkClickFallback(pane.id, requireXtermPaneState(pane.terminal).term, linkDeps)
  )
  refs.httpLinkClickFallbackDisposablesRef.current.set(
    pane.id,
    installHttpLinkClickFallback(requireXtermPaneState(pane.terminal).term, {
      ...linkDeps,
  ...
  refs.selectionDisposablesRef.current.set(
    ...
    pane.terminal.onSelectionChange(() => {
      ...
      if (
        ...
        terminalSelectionExceedsPrimaryLimit(requireXtermPaneState(pane.terminal).term)
      ) {
        return
      }
      if (shouldWritePrimarySelection) {
        ...
        const timer = window.setTimeout(() => {
          ...
          if (terminalSelectionExceedsPrimaryLimit(requireXtermPaneState(pane.terminal).term)) {
            return
          }
  ...
  requireXtermPaneState(pane.terminal).term.options.linkHandler = {
