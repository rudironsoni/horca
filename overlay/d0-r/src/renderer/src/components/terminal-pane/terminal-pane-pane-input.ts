...
export function installTerminalPaneInputHandling(context: PaneInputContext): void {
  ...
  const imeComposerPlaceholderMask = installTerminalImeComposerPlaceholderMask(requireXtermPaneState(pane.terminal).term)
  const iosHangulPreedit = isIosWeb
    ? installTerminalIosHangulPreedit({
        ...
        isScreenReaderMode: () => requireXtermPaneState(pane.terminal).term.options.screenReaderMode === true,
        sendInput: (data) => pane.terminal.input(data),
        renderPreedit: createTerminalIosHangulPreeditRenderer(requireXtermPaneState(pane.terminal).term)
      })
    : null
  ...
  pane.terminal.attachCustomKeyEventHandler((event) => {
    ...
    if (
      ...
      if (event.type === 'keydown') {
        ...
        resetTerminalKeyboardProtocolAfterInterrupt(requireXtermPaneState(pane.terminal).term)
      } else {
        pendingTerminalInterruptKeyup = false
      }
      observeLinuxCandidateEvent()
    ...
    if (event.type === 'keydown') {
      ...
      if (event.key === 'PageUp' || event.key === 'Home') {
        markTerminalPinnedViewport(requireXtermPaneState(pane.terminal).term)
        syncTerminalScrollIntentSoon(requireXtermPaneState(pane.terminal).term, {
          preservePinnedAtBottom: true,
          shouldSync: shouldSyncCurrentTerminal
        })
      } else if (event.key === 'PageDown' || event.key === 'End') {
        syncTerminalScrollIntentSoon(requireXtermPaneState(pane.terminal).term, { shouldSync: shouldSyncCurrentTerminal })
      }
    }
