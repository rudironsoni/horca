...
describe('tui agent startup plans', () => {
  ...
  it('uses the Linux Orca CLI command for Claude Agent Teams launches', () => {
    ...
    expect(plan?.launchCommand).toBe('horca claude-teams')
  })
  ...
  it('uses the plain orca shim for Claude Agent Teams on Linux SSH remotes', () => {
    ...
    // `horca` GNOME-screen-reader workaround), so a remote launch must not
    // emit `horca claude-teams` — that name is not on the remote PATH and
  ...
  it('keeps the Linux horca wrapper for local (non-remote) Claude Agent Teams', () => {
    // Why: the `horca` rename is still required for a local Linux desktop
