...
describe('connectPanePty', () => {
  ...
  it('drops a too-wide daemon alt frame and keeps the scrollback prefix', async () => {
    ...
    const pane = createPane(1)
    pane.terminal.proposeDimensions = vi.fn(() => ({ cols: 120, rows: 40 }))
    const manager = createManager(1)
    const deps = createDeps({
      restoredLeafId: LEAF_1,
      restoredPtyIdByLeafId: { [LEAF_1]: 'tab-pty' }
  ...
  it('keeps a daemon alt frame at its capture grid while the target grid is unknown', async () => {
    ...
    pane.terminal.proposeDimensions = vi.fn(() => ({ cols: 1, rows: 1 }))
    connectPanePty(
      pane as never,
      createManager(1) as never,
      createDeps({
  ...
  it('waits for a recovered destination fit before forwarding its grid or live output', async () => {
    ...
    const pane = createPane(1)
    pane.terminal.proposeDimensions = vi.fn(() => undefined) as never
    ...
    pane.terminal.proposeDimensions = vi.fn(() => ({ cols: 120, rows: 40 })) as never
