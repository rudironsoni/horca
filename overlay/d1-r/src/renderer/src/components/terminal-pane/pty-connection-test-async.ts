...
async function createHeadlessTerm(cols: number, rows: number): Promise<HeadlessTerm> {
  const mod = await import('@xterm/headless')
  return new mod.Terminal({ cols, rows, allowProposedApi: true }) as HeadlessTerm
}

export function writeHeadlessTerminal(term: HeadlessTerm, data: string): Promise<void> {
...
export async function renderHeadlessBuffer(
  ...
  const term = await createHeadlessTerm(cols, rows)
  try {
