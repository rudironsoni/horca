...
function asCapturePane<T extends { terminal: { options: { scrollback: number } }; serializeAddon?: { serialize: (...args: never[]) => string } }>(
  pane: T
): T {
  return attachXtermPaneTestState(pane as never, {
    serializeAddon: (pane.serializeAddon ?? { serialize: () => '', dispose() {} }) as never
  })
}
...
describe.skip('captureTerminalShutdownLayout', () => {
  it('uses the bounded native fit check without a separate JS byte scan', async () => {
    ...
    const serialize = vi.fn(() => contents)
    const pane = attachXtermPaneTestState(
      {
        id: 1,
        leafId: LEAF_ID,
        terminal: mockTerminal(5_000)
      } as never,
      { serializeAddon: { serialize, dispose: vi.fn() } as never }
    )
    try {
      ...
      expect(pane.terminal.serialize).toHaveBeenCalledExactlyOnceWith({ scrollback: 5_000 })
  ...
  it('skips local shutdown scrollback serialization while preserving layout metadata', async () => {
    ...
    expect(pane.terminal.serialize).not.toHaveBeenCalled()
  ...
  it('bounds serialize probes while capping an oversized buffer by UTF-8 bytes', async () => {
    ...
    const pane = attachXtermPaneTestState({
      ...
      stablePaneId: LEAF_ID,
      terminal: mockTerminal(512)
    } as never, { serializeAddon: { serialize } as never })
    const manager = {
      getPanes: vi.fn(() => [pane]),
      getActivePane: vi.fn(() => pane)
    }
