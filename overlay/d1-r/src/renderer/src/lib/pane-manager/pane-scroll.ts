...
function asScrollMarker(
  marker: { dispose: () => void; isDisposed?: boolean; line?: number } | undefined
): { dispose: () => void; isDisposed: boolean; line: number } | undefined {
  if (marker == null || typeof marker.line !== 'number') {
    return undefined
  }
  return {
    dispose: () => marker.dispose(),
    isDisposed: marker.isDisposed === true,
    line: marker.line
  }
}
...
export function captureScrollState(terminal: Terminal): ScrollState {
  ...
  const firstVisibleLineMarker =
    !wasAtBottom && buf.type === 'normal'
      ? asScrollMarker(terminal.registerMarker?.(viewportY - (buf.baseY + buf.cursorY)))
      : undefined
  return {
