...
describe('safeFit', () => {
  ...
  it('coalesces fits until replay parsing and scroll restoration complete', async () => {
    ...
    vi.mocked(pane.terminal.fit).mockImplementation(() => {
  ...
  it.skip('restores pinned content via marker when fit reflow renumbers buffer lines', () => {
    const pane = createPane({
      proposedCols: 100,
      proposedRows: 32,
      terminalCols: 120,
  ...
  it.skip('records the restored post-reflow pin when widening lowers baseY', () => {
    const pane = createPane({
      proposedCols: 160,
      proposedRows: 32,
      terminalCols: 80,
  ...
  it.skip('retries a transient dimensions failure before recording the post-fit pin', () => {
  ...
  it('retains an authoritative continuation until a later measurable fit succeeds', async () => {
    ...
    vi.mocked(pane.terminal.proposeDimensions).mockReturnValue(undefined)
    const continuation = vi.fn()

    const pending = safeFitAndThen(pane, 'pty-resize', continuation)

    expect(continuation).not.toHaveBeenCalled()
    vi.mocked(pane.terminal.proposeDimensions).mockReturnValue({ cols: 100, rows: 32 })
    safeFit(pane)
