...
function fireContextLoss(pane: ManagedPaneInternal): void {
  ...
  const addon = requireXtermPaneState(pane.terminal).webglAddon as unknown as { _onContextLoss: { fire: () => void } }
  addon._onContextLoss.fire()
}

describe.skip('terminal WebGL context recovery', () => {
  beforeEach(async () => {
  ...
  it('retries a backed-off attach on the next rendering resume', () => {
    ...
    expect(requireXtermPaneState(pane.terminal).term.loadAddon).toHaveBeenCalledTimes(2)
  })
  ...
  it('defers retained replay rebuilds until rendering resumes', () => {
    ...
    const pane = createPane()
    attachWebgl(pane)
    const retainedAddon = requireXtermPaneState(pane.terminal).webglAddon
    ...
    expect(requireXtermPaneState(pane.terminal).webglAddon).not.toBe(retainedAddon)
  ...
  it('replaces a retained context lost before xterm dispatches its loss event', () => {
    ...
    requireXtermPaneState(pane.terminal).webglAddon = lostAddon as never

    resumePaneRendering([pane])

    expect(dispose).toHaveBeenCalledTimes(1)
    expect(requireXtermPaneState(pane.terminal).webglAddon).not.toBe(lostAddon)
    expect(requireXtermPaneState(pane.terminal).term.loadAddon).toHaveBeenCalledTimes(1)
  })
  ...
  it('stops resume retries after repeated losses in the retry window', () => {
    ...
    expect(pane.webglDisabledAfterContextLoss).toBe(true)
    expect(requireXtermPaneState(pane.terminal).webglAddon).toBeNull()
    expect(requireXtermPaneState(pane.terminal).term.loadAddon).toHaveBeenCalledTimes(3)
  })
