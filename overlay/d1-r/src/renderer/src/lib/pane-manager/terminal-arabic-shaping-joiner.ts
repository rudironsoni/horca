...
const lazyArabicShapingJoinerByTerminal = new WeakMap<object, LazyArabicShapingJoinerState>()
...
export function registerArabicShapingJoiner(
  ...
  const host = terminal as ArabicShapingTerminal
  const register = host.registerCharacterJoiner
  const deregister = host.deregisterCharacterJoiner
  if (typeof register !== 'function' || typeof deregister !== 'function') {
    return () => undefined
  }
  const joinerId = register((text) => (isShapingActive() ? findRtlJoinRanges(text) : []))
  return () => {
    deregister(joinerId)
  }
}
...
export function ensureArabicShapingJoinerForText(terminal: object, text: string): void {
