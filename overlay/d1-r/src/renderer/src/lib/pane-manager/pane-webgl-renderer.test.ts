...
function createFittablePane(): ManagedPaneInternal {
  ...
  requireXtermPaneState(pane.terminal).fitAddon.proposeDimensions = vi.fn(() =>
    requireXtermPaneState(pane.terminal).webglAddon ? { cols: 84, rows: 24 } : { cols: 80, rows: 24 }
  ) as never
  return pane
}
...
describe.skip('terminal WebGL addon lifecycle', () => {
  beforeEach(async () => {
  ...
  it('disposes a live addon when attach bails instead of orphaning it', () => {
    const pane = createPane()
    attachWebgl(pane)
    const liveAddon = requireXtermPaneState(pane.terminal).webglAddon
  ...
  it('disposes the previous addon before attaching a replacement', () => {
    const pane = createPane()
    attachWebgl(pane)
    const firstAddon = requireXtermPaneState(pane.terminal).webglAddon
    ...
    expect(requireXtermPaneState(pane.terminal).webglAddon).not.toBe(firstAddon)
  })
  ...
  it('keeps attaching to healthy panes after another pane fails to attach', () => {
    ...
    expect(requireXtermPaneState(healthy.terminal).webglAddon).not.toBeNull()
...
describe.skip('fit-anchored WebGL reattach', () => {
  beforeEach(() => {
    resetTerminalWebglSuggestion()
    vi.spyOn(console, 'warn').mockImplementation(() => {})
    vi.stubGlobal('requestAnimationFrame', (callback: FrameRequestCallback) => {
  ...
  it('does not disturb a pane that already has a live addon', () => {
    ...
    expect(requireXtermPaneState(pane.terminal).webglAddon).toBe(liveAddon)
  })
})
...
describe.skip('safeFit drives the fit-anchored WebGL reattach', () => {
  beforeEach(() => {
    resetTerminalWebglSuggestion()
    vi.spyOn(console, 'warn').mockImplementation(() => {})
    vi.stubGlobal('requestAnimationFrame', (callback: FrameRequestCallback) => {
  ...
  it('refits the pane onto the WebGL cell metrics after attaching', () => {
    ...
    requireXtermPaneState(pane.terminal).fitAddon.fit = vi.fn(() => {
      hadAddonPerFit.push(requireXtermPaneState(pane.terminal).webglAddon != null)
    }) as never
...
describe.skip('the deferred fit-anchored refit frame', () => {
...
describe.skip('deferred WebGL addon load', () => {
  beforeEach(() => {
    vi.resetModules()
    vi.spyOn(console, 'warn').mockImplementation(() => {})
    vi.stubGlobal('requestAnimationFrame', (callback: FrameRequestCallback) => {
  ...
  it('refits a pane that attached while the addon was still loading', async () => {
    ...
    expect(requireXtermPaneState(pane.terminal).webglAddon).not.toBeNull()
    expect(requireXtermPaneState(pane.terminal).fitAddon.fit).toHaveBeenCalled()
  })
