...
import { WideCellGrid } from './__fixtures__/terminal-wide-cell-grid'
...
function paint(cols: number, sequence: string): Painted {
  ...
  return { rows: emulator.getVisibleLines().map((line) => line.replace(/\s+$/, '')), emulator }
}
  ...
  it('reflows to exactly what the target width would have drawn from scratch', () => {
    ...
    for (let cols = 12; cols <= 48; cols += 1) {
      for (const target of [cols - 5, cols - 1, cols + 1, cols + 7]) {
        ...
        const reflowed = emulator.getVisibleLines().map((line) => line.replace(/\s+$/, ''))
        emulator.dispose()
