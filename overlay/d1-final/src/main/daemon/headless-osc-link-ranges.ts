type IBufferCell = { extended?: { urlId?: number }; hasExtendedAttrs?: () => boolean }
type IBufferLine = { length: number; getCell: (x: number, cell?: IBufferCell) => IBufferCell | undefined }
type Terminal = {
  cols: number
  rows: number
  buffer: {
  ...
  _core?: { _oscLinkService?: { getLinkData: (id: number) => { uri?: string } | undefined; _dataByLinkId?: { size?: number } } }
}
